...
### Download vcf
vcf <- vcfR::read.vcfR("../00-data-dryad/831snps_241ind.recode.vcf")

### Download pop info
pop <- read.table("../00-data/population_map_palinurus_241ind_mpa.txt", header=TRUE, sep="\t", stringsAsFactors = TRUE)
...
set.seed(10); grp_all <- find.clusters(genind, max.n.clust=10, n.pca=nInd(genind)/3) # select N/3
# Select K =2
...
### Calculate percent of males and females in clusters
dapc_compoplot_sex_ggplot %>% group_by(CLUSTER, SEX,value) %>%
  tally()
...
### Save the graph
ggsave("Figure3_241ind.pdf", width=10, height=5)
...
ggplot(dapc_sex_all_info, aes(x="", y=K, fill=K)) +
  ...
  scale_fill_manual(values=col2)+
  xlab("")
ggsave("Male_female_piechart_dapc_241ind.pdf")
...
admixture <- read.table("plink.2.Q")
...
### How many female and males in both cluster
admixture_sex %>% group_by(CLUSTER, SEX) %>% tally()
