...
ggsave("Fig3.pdf", width=5, height=10)
...
env_M <- pca_axis[which(rownames(pca_axis) %in% male),]
env_F <- pca_axis[which(rownames(pca_axis) %in% female),]
...
# Create one dataset for males and one for females
mpa_dummy_M <- mpa_dummy[which(rownames(mpa_dummy) %in% male),]
mpa_dummy_F <- mpa_dummy[which(rownames(mpa_dummy) %in% female),]
...
habitats_dummy <- habitats_dummy[which(rownames(habitats_dummy) %in% row.names(genind@tab)),]

# Create one dataset for males and one for females
habitats_dummy_M <- habitats_dummy[which(rownames(habitats_dummy) %in% male),]
habitats_dummy_F <- habitats_dummy[which(rownames(habitats_dummy) %in% female),]
...
X_M <- X_M[which(rownames(X_M) %in% rownames(env_M)),]
X_F <- X_F[which(rownames(X_F) %in% rownames(env_F)),]
...
rda0envm<-rda(X_M ~ 1, env_M[,1:3])
rda0envf<-rda(X_F ~ 1, env_F[,1:3])

#OrdiR2step will move towards the global model with all explanatory variables
rdaGenvm<- rda(X_M ~ ., env_M[,1:3])
rdaGenvf<- rda(X_F ~ ., env_F[,1:3])

### Select the variables
Selenvm <- ordistep(rda0envm, scope=formula(rdaGenvm), direction="both") 
Selenvf <- ordistep(rda0envf, scope=formula(rdaGenvf), direction="both") 
...
############### SELECT MEM ####
Coor=select(mpa, LAT, LON)
Coorxy=select(mpa, LON,LAT)
...
mem_select_F <- dbmem_F %>% select(MEM1:MEM3)
mem_select_M <- dbmem_M %>% select(MEM1:MEM5)
...
rdaPC1<- rda(X_M, Ysel_envM[,1])
RsquareAdj(rdaPC1)
anova(rdaPC1, perm=999)

# MEM2 versus PC3
rdaMEM2<- rda(X_M, mem_select_M$MEM2)
RsquareAdj(rdaMEM2)
anova(rdaMEM2, perm=999)

rdaPC3<- rda(X_M, Ysel_envM[,3])
RsquareAdj(rdaPC3)
...
rdaPC1<- rda(X_F, Ysel_envF[,1])
RsquareAdj(rdaPC1)
anova(rdaPC1, perm=999)

# MEM2 versus PC3
rdaMEM2<- rda(X_F, mem_select_F$MEM2)
RsquareAdj(rdaMEM2)
anova(rdaMEM2, perm=999)

rdaPC3<- rda(X_F, Ysel_envF[,3])
RsquareAdj(rdaPC3)
anova(rdaPC3, perm=999)
...
### Add mpa names
sitenames<- mpa[which(mpa$IND %in% rownames(scrs$sites)), "MPA"]
