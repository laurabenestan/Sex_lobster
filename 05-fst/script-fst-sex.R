...
write.table(fst_female$pairwise.fst,"Fst_female.txt",sep="\t", row.names = FALSE, quote=FALSE)

### Extract the FST matrix
fst.female.matrix <- fst_female$pairwise.fst.full.matrix

### Extract the FST overall markers values
fst_female$fst.overall

### Remove the _ character
fst.female.matrix  <- gsub("_", " ", fst.female.matrix)

### Melt the FST matrix to produce the nice graph
melted_fst_female <- reshape::melt(fst.female.matrix,na.rm = FALSE)

### Verify that the Fst values is indeed a numerical value
melted_fst_female$value <- as.numeric(as.character(melted_fst_female$value))

### Remove NA
melted_fst_female <- na.omit(melted_fst_female)
melted_fst_female$value <- as.numeric(as.character(melted_fst_female$value))

### Make the graph
female <-ggplot2::ggplot(data = melted_fst_female, aes(X2, X1, fill = value))+
...
male_female <- cbind(melted_fst_male,melted_fst_female)
...
male_female_fst_long %>%
  group_by(SEX) %>%
summarize(mean=mean(FST),min=min(FST), max=max(FST))
