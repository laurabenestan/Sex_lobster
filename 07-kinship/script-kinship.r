...
kinship <- read.table("kinship-241ind.txt",header=TRUE)
...
# sample size
sample_size = sex_indv_no_self_no_na %>% 
  group_by(SEX_CAT) %>% 
  summarize(num=n())
...
sex_indv_no_self_no_na_subset %>%
  ...
  theme_bw() +
  ylab("Relatedness (Index of Loiselle)")+
...
sex_indv_no_self_no_na_subset %>%
  ggline(x = "SEX_CAT", y = "KINSHIP", 
         add = c("mean_se"), 
         ylab = "Relatedness (Index of Loiselle)", xlab = "Comparison between genders")
